...
class CudaPcgWorkspace {
 ...
 private:
  ...
  friend CudaCgSolveResult SolveNormalEquationsPcg(
      ...
      const CudaGridShape& grid_shape, int max_iterations, float tolerance);
};
...
CudaCgSolveResult SolveNormalEquationsPcg(
    ...
    int num_grid_vals_per_component, int max_iterations, float tolerance);
